  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-10 md:py-14">
      <div className="flex items-center gap-2 text-[13px] text-[#86868b]">
        <Link href="/blog" className="hover:text-[#1d1d1f] transition">
          Blog
        </Link>
        <span>/</span>
      ...
      <div className={`mt-5 relative h-52 md:h-72 rounded-[36px] overflow-hidden ${coverBg(post.coverColor)} flex items-center justify-center apple-shadow`}>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.35),transparent_55%)]" />
        <span className="flex items-center justify-center w-28 h-28 rounded-[34px] bg-white/90 text-[#1d1d1f] shadow-sm">
      ...
      <article className="mt-8 prose prose-lg max-w-none text-[17px] leading-[1.75] text-[#1d1d1f]/90">
        {post.body.split("\n").map((par, i) => (
          ...
          <div className="mt-6 grid sm:grid-cols-3 gap-4">
            {more.map((m) => (
              <Link
                key={m.id}
                href={`/blog/${m.slug}`}
                className="group rounded-[24px] bg-white border border-[#123f34]/[0.08] p-4 card-lift"
              >
                <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#f7f2ed] text-[#006b55] ring-1 ring-[#123f34]/5">
